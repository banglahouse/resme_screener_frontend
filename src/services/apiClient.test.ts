...
describe('apiClient', () => {
  test('uploadApplication sends multipart payload with auth headers', async () => {
    server.use(
      rest.post(`${apiUrl}/applications`, async (req, res, ctx) => {
  ...
  test('uploadApplication throws ApiError on failure', async () => {
    server.use(
      rest.post(`${apiUrl}/applications`, (_req, res, ctx) =>
        res(ctx.status(400), ctx.json({ message: 'Missing data', statusCode: 400 }))
      )
    );
    ...
    await expect(
      uploadApplication({ jobKey: 'ENG-1', candidateUserId: 'cand-1', resumeFile: file, jdFile: jd })
    ).rejects.toMatchObject({ message: 'Missing data', statusCode: 400 });
  });
  ...
  test('listChats and sendChat parse payloads', async () => {
    server.use(
      rest.get(`${apiUrl}/applications/:id/chats`, (_req, res, ctx) =>
        res(
          ctx.status(200),
          ctx.json({ messages: [{ id: '1', role: 'assistant', content: 'Hi', createdAt: new Date().toISOString(), applicationId: 'app-1' }], total: 1, limit: 50, offset: 0 })
        )
      ),
      rest.post(`${apiUrl}/applications/:id/chat`, async (req, res, ctx) => {
