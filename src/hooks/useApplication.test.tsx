...
const apiUrl = '*/api';
const wrapper = ({ children }: { children: ReactNode }) => <ApplicationProvider>{children}</ApplicationProvider>;

describe('useApplication', () => {
  test('submitApplication hydrates application and chats', async () => {
    server.use(
      rest.post(`${apiUrl}/applications`, async (_req, res, ctx) =>
        res(
          ctx.status(200),
          ctx.json({
      ...
      rest.get(`${apiUrl}/applications/:id/chats`, (_req, res, ctx) =>
        res(
          ctx.status(200),
          ctx.json({ messages: [{ id: '1', role: 'assistant', content: 'Insight', createdAt: new Date().toISOString(), applicationId: 'app-123' }], total: 1, limit: 50, offset: 0 })
        )
      )
    );
  ...
  test('loadApplicationById fetches existing application', async () => {
    server.use(
      rest.get(`${apiUrl}/applications/:applicationId`, (req, res, ctx) =>
        res(
          ctx.status(200),
          ctx.json({ applicationId: req.params.applicationId, jobKey: 'ENG-1', match: { score: 75, strengths: [], gaps: [], insights: [] } })
        )
      ),
      rest.get(`${apiUrl}/applications/:applicationId/chats`, (_req, res, ctx) =>
        res(
          ctx.status(200),
  ...
  test('sendMessage appends optimistic and assistant replies', async () => {
    server.use(
      rest.post(`${apiUrl}/applications/:applicationId/chat`, (_req, res, ctx) =>
        res(
          ctx.status(200),
          ctx.json({
            answer: 'Reply',
